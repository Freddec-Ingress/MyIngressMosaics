	...
	for imcountry_obj in imcountry_results:
		...
		for imregion_obj in imregion_results:
			...
			for imcity_obj in imcity_results:
				...
				immosaic_results = IMMosaic.objects.filter(country_name=imcountry_obj.name, region_name=imregion_obj.name, city_name=imcity_obj.name).exclude(registered=True).order_by('-count', 'name')
				for immosaic_obj in immosaic_results:
